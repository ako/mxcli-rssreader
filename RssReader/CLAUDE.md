...
```bash
./mxcli -p RssReader.mpr    # Correct - uses local binary
```
...
```bash
./mxcli setup mxbuild -p RssReader.mpr    # Auto-download for project's Mendix version
```

After setup, `mx` is at `~/.mxcli/mxbuild/{version}/modeler/mx`. Usage:

```bash
~/.mxcli/mxbuild/*/modeler/mx check RssReader.mpr   # Validate project
./mxcli docker check -p RssReader.mpr               # Alternative (auto-downloads mxbuild)
```
...
```bash
./mxcli -p RssReader.mpr -c "SHOW MODULES"              # List all modules
./mxcli -p RssReader.mpr -c "SHOW STRUCTURE"             # Project overview
./mxcli -p RssReader.mpr -c "SHOW ENTITIES IN MyModule"  # Entities in a module
./mxcli -p RssReader.mpr -c "DESCRIBE ENTITY MyModule.Customer"  # Entity details
```

### Execute an MDL Script File

```bash
./mxcli exec script.mdl -p RssReader.mpr
```
...
```bash
./mxcli
# Then: CONNECT LOCAL 'RssReader.mpr';
```
...
./mxcli check script.mdl -p RssReader.mpr --references  # With reference validation
```
...
```bash
./mxcli check script.mdl -p RssReader.mpr --references
```
...
```bash
# Lint the project
./mxcli lint -p RssReader.mpr

# With colored output
./mxcli lint -p RssReader.mpr --color

# List available rules
./mxcli lint -p RssReader.mpr --list-rules

# Output as SARIF
./mxcli lint -p RssReader.mpr --format sarif > results.sarif
```
...
./mxcli report -p RssReader.mpr

# JSON report
./mxcli report -p RssReader.mpr --format json

# HTML report
./mxcli report -p RssReader.mpr --format html
```
